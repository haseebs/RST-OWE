...
from owe.models.open_world.transformation import LinearTransform, FCNTransform, RelationBasedTransform
...
class Mapper(torch.nn.Module):
    ...
    """
    def __init__(self, embedding_m: torch.tensor, complex_d: int, num_matrices: int):
        super().__init__()
        ...
        if Config.get("LinkPredictionModelType") in ("ComplEx", "RotatE"):
            ...
            elif Config.get("TransformationType") == "RelationBased":
                self.transformer_r = RelationBasedTransform(encoder_out_d, complex_d, bias=True, num_mat=num_matrices)
                self.transformer_i = RelationBasedTransform(encoder_out_d, complex_d, bias=True, num_mat=num_matrices)
                logger.info("Using Relationship based transformation")
    ...
    def forward(self, name: torch.tensor, description: torch.tensor, relation: int):
        enc = self.encoder(name, description)

        if Config.get("TransformationType") == "RelationBased":
            return self.transformer_r(enc, relation), self.transformer_i(enc, relation)
        elif Config.get("LinkPredictionModelType") in ("ComplEx", "RotatE"):
